export default {
    ...
    bugreport: {
        export: 'Экспорт Bugreport',
        exporting: 'Экспорт…',
        export_success_title: 'Bugreport экспортирован',
        export_success: 'Bugreport сохранен в:',
        export_failed_title: 'Ошибка экспорта',
        export_failed: 'Не удалось экспортировать Bugreport.',
        copy_path: 'Скопировать путь',
        copy_path_failed: 'Не удалось скопировать',
    ...
    zygote_state: {
        ...
        abnormal_desc: 'Zygisk был внедрён в процесс Zygote {pid}, но JNI Hook не нашёл подходящих методов ({funcs}), поэтому модули не смогут нормально загружаться. Это может быть новая неподдерживаемая версия или особая система Android. Создайте Issue на GitHub и предоставьте /system/framework/framework.jar и /system/lib64/libandroid_runtime.so из текущей системы, чтобы разработчики могли добавить поддержку.'
    },
    corrupted: {
    ...
    module: {
        issue: {
            ...
            linker_issue: 'Модуль {name} не удалось загрузить. Сообщите о проблеме разработчику модуля.',
            crash_issue: 'Модуль {name} обнаружен в обратной трассировке аварийного потока: {processes}.',
